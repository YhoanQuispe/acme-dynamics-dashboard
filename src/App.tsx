...
export default function App() {
  ...
  const [orders, setOrders] = useState<Order[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_orders_global_v2');
      if (saved) {
        try {
          ...
          console.error('Error loading orders from safeStorage v2', e);
        }
      }
    }
  ...
  const [items, setItems] = useState<InventoryItem[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_inventory_global_v2');
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          console.error('Error loading inventory from safeStorage v2', e);
        }
      }
    }
  ...
  // Persists to safeStorage whenever modified
  useEffect(() => {
    safeStorage.setItem('erp_orders_global_v2', JSON.stringify(orders));
  }, [orders]);

  useEffect(() => {
    safeStorage.setItem('erp_inventory_global_v2', JSON.stringify(items));
  }, [items]);
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      try {
        ...
        console.warn('matchMedia is not supported in this frame environment', e);
      }
    }
    return 'light';
  });
  ...
  const [alertSettings, setAlertSettings] = useState<AlertSettings>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_alert_settings');
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (
          ...
          console.error('Error parsing erp_alert_settings from safeStorage', e);
        }
      }
    }
  ...
  useEffect(() => {
    safeStorage.setItem('erp_alert_settings', JSON.stringify(alertSettings));
    if (!alertSettings.systemAlerts) {
      setNotifications([]);
    }
  }, [alertSettings]);

  // Synchronized operator profile state from safeStorage with fallback
  const [operatorProfile, setOperatorProfile] = useState<OperatorProfile>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_operator_profile');
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          ...
          console.error('Error parsing erp_operator_profile from safeStorage', e);
        }
      }
    }
  ...
  const [blueprints, setBlueprints] = useState<Blueprint[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_blueprints');
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          ...
          console.error('Error parsing blueprints from safeStorage', e);
        }
      }
    }
    return [
  ...
  // Sync blueprints with safeStorage
  useEffect(() => {
    safeStorage.setItem('erp_blueprints', JSON.stringify(blueprints));
  }, [blueprints]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 font-sans text-zinc-900 dark:text-zinc-50 antialiased selection:bg-zinc-900/10 dark:selection:bg-zinc-100/10 flex transition-colors duration-200">
      ...
      <aside 
        className={`fixed inset-y-0 left-0 z-40 flex flex-col border-r border-zinc-200/60 dark:border-zinc-800/50 bg-white dark:bg-zinc-950 transition-all duration-300 ease-in-out transform ${
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-zinc-100/80 dark:border-zinc-800/50 shrink-0">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="h-8 w-8 rounded-lg bg-indigo-600 dark:bg-indigo-500 flex items-center justify-center shrink-0 shadow-sm shadow-indigo-500/20">
          ...
          {!sidebarCollapsed && (
            <button 
              ...
              className="p-1 rounded-md text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-900/60 hidden md:block cursor-pointer transition-colors duration-200 ease-in-out"
            >
              <ChevronLeft size={16} />
            </button>
          )}
        ...
        {sidebarCollapsed && (
          <div className="flex justify-center py-4 border-b border-zinc-100/80 dark:border-zinc-800/50 shrink-0">
            <button 
              onClick={() => setSidebarCollapsed(false)}
              className="p-1.5 rounded-lg bg-zinc-50 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 cursor-pointer hover:shadow-xs transition-transform duration-200 ease-in-out"
            >
          ...
          {navigationItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full group flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-semibold cursor-pointer transition-all duration-200 ease-in-out ${
                  isActive
                    ? 'bg-indigo-50/80 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300'
                    : 'text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 hover:bg-zinc-50/80 dark:hover:bg-zinc-900/40'
                }`}
                title={sidebarCollapsed ? item.name : undefined}
              >
                <IconComponent 
                  size={16} 
                  className={`shrink-0 transition-transform duration-200 ease-in-out group-hover:scale-110 ${
                    isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-zinc-400 group-hover:text-zinc-900 dark:text-zinc-500 dark:group-hover:text-zinc-300'
                  }`} 
                />
                {!sidebarCollapsed && (
                  <div className="flex flex-1 items-center justify-between truncate">
                    ...
                    {item.id === 'orders' && (
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold tracking-widest uppercase ml-2 border ${isActive ? 'bg-indigo-100 text-indigo-700 border-indigo-200 dark:bg-indigo-900/50 dark:text-indigo-300 dark:border-indigo-800/50' : 'bg-zinc-100 text-zinc-500 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700'}`}>
                        5
                      </span>
                    )}
                    {item.id === 'inventory' && (
        ...
        <div className="border-t border-zinc-100/80 dark:border-zinc-800/50 p-3 shrink-0">
          <div 
          ...
          >
            <div className="relative shrink-0">
              ...
              <div className="h-9 w-9 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center font-bold text-indigo-600 dark:text-indigo-400 text-xs border border-indigo-200/60 dark:border-indigo-800/50">
                {getInitials(operatorProfile.name)}
              </div>
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${
        ...
        <header className="sticky top-0 z-30 h-16 border-b border-zinc-200/60 dark:border-zinc-800/50 bg-white/85 dark:bg-zinc-950/85 backdrop-blur-md flex items-center justify-between px-6 transition-all duration-300">
          ...
          <div className="flex items-center gap-4 flex-1 max-w-md">
            ...
            <button 
              ...
              className="p-1.5 rounded-lg text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 block md:hidden cursor-pointer transition-colors duration-200 ease-in-out"
            >
              <Menu size={18} />
            </button>
            ...
            >
              ...
              <input
                ref={searchInputRef}
                type="text"
                placeholder="Search commands, orders, users..."
                value={searchQuery}
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              onClick={toggleTheme}
              className="p-2 rounded-lg text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50 hover:bg-zinc-50 dark:hover:bg-zinc-900/60 cursor-pointer border border-zinc-100/50 dark:border-zinc-800 transition-all duration-200 ease-in-out shadow-xs hover:shadow-sm"
            ...
            <div className="relative">
              <button 
                ...
                className="p-2 rounded-lg text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50 hover:bg-zinc-50 dark:hover:bg-zinc-900/60 cursor-pointer border border-zinc-100/50 dark:border-zinc-800 transition-all duration-200 ease-in-out shadow-xs hover:shadow-sm"
              >
                <Bell size={15} className={`${countUnread > 0 ? 'animate-bounce' : ''}`} />

...
export default function SettingsView({ 
  ...
  const [profileRole, setProfileRole] = useState<'Global Operations Administrator' | 'Logistics Director'>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_operator_role');
  ...
  const [profileDescription, setProfileDescription] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_operator_description');
  ...
  const [syncFrequency, setSyncFrequency] = useState<'15m' | '1h' | '12h' | '24h'>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_pipeline_settings');
  ...
  const handleFrequencyChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setTimeout(() => {
      ...
      safeStorage.setItem('erp_pipeline_settings', value);
  ...
  // Hook to instantly save any changes to alertSettings to safeStorage
  useEffect(() => {
  ...
  const saveSettings = (e: React.FormEvent) => {
    ...
    if (activeTab === 'profile') {
      ...
      setTimeout(() => {
        ...
        safeStorage.setItem('erp_operator_profile', JSON.stringify(updated));
        safeStorage.setItem('erp_operator_role', profileRole);
        safeStorage.setItem('erp_operator_description', profileDescription);

...
export default function CustomersView() {
  ...
  const [customers, setCustomers] = useState<Customer[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeStorage.getItem('erp_customers');
      if (saved) {
        try {
          ...
          console.error('Error loading custom customers from safeStorage', e);
        }
      }
    }
  ...
  // Sync custom customers to safeStorage
  useEffect(() => {
    const customCustomers = customers.filter(c => c.id.startsWith('CST-'));
    safeStorage.setItem('erp_customers', JSON.stringify(customCustomers));
  }, [customers]);
